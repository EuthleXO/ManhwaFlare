...
from Manhwaflare.config import MONGODB_URI, DATABASE_NAME, OWNER_ID, OWNER_IDS, LOG_CHANNEL_ID
...
async def get_or_create_referral(uid: int) -> str:
    ...
    await get().referrals.insert_one({
        "code": code, "owner_id": uid, "uses": 0, "created_at": utcnow(),
    })
    return code


async def apply_referral(uid: int, code: str) -> str:
    """Returns ok / invalid / self / already."""
    code = (code or "").strip().upper()
    doc = await get().referrals.find_one({"code": code})
    if not doc:
        return "invalid"
    if int(doc["owner_id"]) == uid:
        return "self"
    ...
    await get().users.update_one(
        {"user_id": uid},
        {"$set": {"referred_by": doc["owner_id"]}},
    )
    await get().referrals.update_one({"code": code}, {"$inc": {"uses": 1}})
    # bonus +3 daily for both
    today = utcnow().strftime("%Y-%m-%d")
    for target in (uid, int(doc["owner_id"])):
        await ensure_user(target)
        await get().users.update_one(
            {"user_id": target},
            {"$inc": {"daily_used": -3}},  # effectively +3 quota
        )
    return "ok"
